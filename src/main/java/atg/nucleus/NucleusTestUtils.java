 ...
 * NucleusTestUtils
 * @author adamb
 * @version $Id: //test/UnitTests/base/main/src/Java/atg/nucleus/NucleusTestUtils.java#16 $
 * 
...
public class NucleusTestUtils {
 ...
 /**
   ...
   *
   *
   *  This method always creates the config/data subdirectory if
   *  it does not exist.
   * 
   *
  ...
  public static File getConfigpath(Class pClassRelativeTo, String pBaseConfigDirectory, boolean pCreate) {
    ...
    if (fileFound == null) {
      ...
      if (dataURL != null) {
        fileFound = new File(dataURL.getFile());
   ...
   * "Publishing.base", etc). Additionally adds a directory calculate relative
  ...
  public static Nucleus startNucleusWithModules(
    ...
    String strModulesString = StringUtils.joinStringsWithQuoting(
      pModules, File.pathSeparatorChar);
    ...
    try {
      ...
      String configpath = DynamoServerLauncher.calculateConfigPath(
        launcher, false, null, false, null);
      ...
      File fileTestConfig = getConfigpath(
        pClassRelativeTo, pBaseConfigDirectory, false);
