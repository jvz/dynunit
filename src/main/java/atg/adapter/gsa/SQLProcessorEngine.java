	...
	extends GenericService
{
  ...
  //---------- Property: DataSource ----------
  /** DataSource from which to get DB connections
   *  this property is NOT a public property because it is
   *  extracted from the repository property */
  ...
  /** String executed to determine whether a table exists. The table
  ...
  /** String executed to drop a table.  The table name is appended to the
  ...
  /** String delimiter used to separate the large String passed to
  ...
  //-------------------------------------
  /**
   * Close a DB connection, logging any SQLExceptions. It is okay to pass a
   * null connection here
  ...
  //-------------------------------------
  /**
   * Close a result set, logging any SQLExceptions. It is okay to pass a
   * null here
  ...
  //-------------------------------------
  /**
   * Close a statement, logging any SQLExceptions. It is okay to pass a
   * null here.
  ...
  //-------------------------------------
  /**
   * Perform the specified SQL statement in a
   * new transaction which is commited.
   * @param pSQL SQL to execute
   * @return the # of rows affected
   * @exception SQLProcessorException if there is DB or xact trouble
   **/
  ...
  /** This method is used to create tables in a database.  It takes a String
   * that contains all of the table creation statements and is of the format:
   *
   * CREATE TABLE foo (
   ...
   * );
   *
   * Specifically, this is the format output by the GSA when a call is
   * made to generateSQL();
   *
   * Before making the tables, this large String will be split apart
   * into an array of individual CREATE TABLE statements using the
   * createTableBeginDelimiter and createTableEndDelimiter properties.
   * By default,
   * createTableBeginDelimiter = "CREATE TABLE"
   * and
	  ...
	  throws SQLProcessorException
  {
	  ...
	  while ( iter.hasNext() ) {
			...
			if (name != null && !exists ) {
				if ( isLoggingDebug() ) logDebug("Creating table: " + name);
				...
				if ( this.getRepository() instanceof InitializingVersionRepository ) {
					if ( ! isLoggingDebug() &&
						 ((InitializingVersionRepository)this.getRepository()).isLoggingCreateTables() )
						logDebug( statement );
				}				
				...
				if ( isLoggingInfo() )
					logInfo("Table already exists and dropTablesIfExist is false - not creating: " + name );
					// dropExistingTables must be false or else table would have been dropped
			} else {
				//throw new SQLProcessorException("The table " + name + " was not created because name was null or table couldn't be dropped.");
        logWarning("The table " + name + " was not created because name was null or table couldn't be dropped.");
			}
		}
	...
	/**
	 * This is a method that is used to execute a 'CREATE TABLE' call.
	 * The String you pass in is expected to be of the format
	 *
	 *    CREATE TABLE (
	 *          .....
	 *    )
	 *
	 * @return void
	 * @exception SQLProcessorException thrown if an error occurs creating
	 * the table
	 */
	  ...
	  throws SQLProcessorException
		...
		try {
			...
			throw new SQLProcessorException("Caught exception executing create table statement \"" + pStr + "\"", spe);
		}
        }

	/** This method is used to iteratively drop tables.  The iterative
	 * effort is necessary because tables may have references.
	 * ASSUMPTION: references only exist for tables that are defined
	 * within this repository.  If references exist from tables outside this
	 * repository, this method will throw a SQLProcessorException
	 *
	 ...
	 */
  	...
  	/**
	 * This is a method that is used to execute a 'Drop Table' call. The
	 ...
	 * @exception SQLProcessorException thrown if an error occurs trying
	 * to drop the table
	 */
		...
		try {
			...
			throw new SQLProcessorException("Caught exception executing drop table statement \"" + sql + "\"", spe );
		}
         }

  	/**
	 * This method is used to extract table names from a Vector of CREATE statements
	 * returned by either a call to getCreateStatements() or getOrderedCreateStatements()
	 *
	 ...
	 * statement.  It operates by finding the createTableBeginDelimiter
	 * and extracting the next word after the delimiter.
	 *
	...
	private String getTableName( String pStr ) {
		...
		if ( isLoggingDebug() && (this.getRepository().getDebugLevel() > 6) ) {
			...
			logDebug("Name is taken as word after createTableBeginDelimiter.  Delimiter is set to: " + getCreateTableBeginDelimiter() );
		}
	...
	/**
	 * This method is used to break the large string passed to
	 * createTables() into an array of CREATE TABLE
	 * statements.  The string is split apart using the createTableBeginDelimiter
	 * and createTableEndDelimiter Strings.  These can be set as the
	 * createTableBeginDelimiter and createTableEndDelimiter properties.
	 *
	 * @param String - String containing all the Create Table statements
	 * @return Vector of CREATE TABLE statements
	 */
/*	private Vector getCreateStatements( String pStr ) {
		String STATEMENT_BEGIN = getCreateTableBeginDelimiter();
		String STATEMENT_END = getCreateTableEndDelimiter();
		...
		// string being processed has no CREATE TABLE clauses
		int index = pStr.indexOf( STATEMENT_BEGIN );

		// now loop through and extract all of the CREATE statements
		String remaining = pStr.substring( index );
		while ( index != -1 ) {
			int stop = remaining.indexOf( STATEMENT_END );
			if ( stop == -1 ) {
				// error - this string is malformed wrt what we expected because no end delimiter was found
				if ( isLoggingError() )
					logError("malformed string passed to getCreateStatements - an end delimiter '" + STATEMENT_END + "' could not be found. Abandoning parsing of Create table statements.");
				break;
			}

			String thisCreate = remaining.substring(0,stop);
			if ( isLoggingDebug() )
				logDebug("Parsed create statement: " + thisCreate );
			statements.add( thisCreate );

			// now see if there are any more statements
		        remaining = remaining.substring( stop + 1 );
			index = remaining.indexOf( STATEMENT_BEGIN );
                        // need to put this here so that we skip everything between the end of the previous
                        // CREATE TABLE and the beginning of the next
                        if ( index != -1 )
                             remaining = remaining.substring( index );
		}

		if ( isLoggingDebug() )
			logDebug("Found " + statements.size() + " create statements.");

		return statements;
	}
*/

  /** This method is used to order CREATE TABLE statments such that we
   * do not try to create a table before any tables that it references.
   * NOTE: if a reference exists for a table outside of this repository
   * we will print a warning, but will _not_ throw an exception.  If the
   * referenced table doesn't exist, an exception will be thrown when the
   * referencing table is created.
   *
   * @param String containing all of the CREATE TABLE statements as generated
   * by a call to GSARepository.generateSQL()
   * @return Vector of individual CREATE statements that are in
   * the proper order to execute
		...
		throws SQLProcessorException
	{
		...
		while ( iter.hasNext() ) {
			...
			if ( references.size() < 1 ) {
				...
				if ( ! checkReferencesInRepository( names, references ) ) {
					if ( isLoggingWarning() )
						logWarning("Table " + tableName + " references a table outside the repository.");
				}
		...
		while ( statements.size() > 0 ) {
			...
			if ( attempt++ > maxTries ) {
				...
				throw new SQLProcessorException("Could not order tables appropriately...failing.  Turn on loggingDebug for more info.");
			}
		}
  ...
  /** This method is used to extract the names of other tables that a
   * table references.  expected format is:
   *
   *   CREATE TABLE foo (
   *	  x int not null references bar(id),
   *      y varchar null references doo(id),
   *      ...
   *   )
   *
   * @param String the CREATE TABLE statement
   ...
   */
		...
		throws SQLProcessorException
	{
		...
		while ( start != -1 ) {
			...
			// bail if the table references itself - i think that is just wrong
                        /* actually, jeff and mike said this should be allowed, so i won't throw an
                           exception, i just won't add it to the list of references either... */
			if ( ref.equalsIgnoreCase( tableName ) ) {
                            // do nothing
				//throw new SQLProcessorException("The create statement for table " + tableName +
	...
	/** This method is used to determine whether all the items in
	 * the second Vector are contained in the first Vector.
	 *
 ...
 /* These methods are really used - they were just part of mgk's original class */
   ...
   *
   * @beaninfo description: SQL to insert a new id space into the DB
   * table
   * @return InsertSQL
   **/
   ...
   *
   * @beaninfo description: SQL to execute to update a specific id
   * space int the D0B
   * @return UpdateSQL
   **/
   ...
   *
   * @beaninfo description: SQL to execute to load a specific id
   * space from the DB
   * @return SelectSQL
   **/
