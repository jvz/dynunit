...
echo DYNAMO_HOME is not set. Please set it. For example: set DYNAMO_HOME=C:\ATG\ATG2007.1\home
goto :end
...
REM Assuming 2007.1p1
echo Installing DAS/lib/classes.jar

cd %DUST_HOME%
call %MAVEN_HOME%\bin\mvn deploy:deploy-file -DgroupId=atg -DartifactId=DAS -Dversion=2007.1p1 -Dpackaging=jar -Dfile=%DYNAMO_HOME%/../DAS/lib/classes.jar -DgeneratePom=true -Durl=file://c:/maven/repository -DlocalRepository=local

echo Installing DAS/lib/resources.jar

cd %DUST_HOME%
call %MAVEN_HOME%\bin\mvn deploy:deploy-file -DgroupId=atg -DartifactId=das-resources -Dversion=2007.1p1 -Dpackaging=jar -Dfile=%DYNAMO_HOME%/../DAS/lib/resources.jar -DgeneratePom=true -Durl=file://c:/maven/repository -DlocalRepository=local
